...
R = "\033[31m"   
C = "\033[36;1m"
os.system("rm -rf .unpost.py ;rm -rf /sdcard/download/.unpost.py")
joker = "3"
...
def chatMS():
	def aco():
		...
		if '"status":"ok"' in g.text:
			print(f'    chat deleted -> {foCU}')
		
		else:
			print('    band bwy kateki tr ama bka ! ')
...
def post():
	def pos():
		...
		if '"status":"ok"' in dl.text:
			print(f' '+wd+'    Deleted [ id post > {iid}]')
		else:
